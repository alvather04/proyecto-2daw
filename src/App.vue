<template>
  <div id="app">
    <NavBar :menuOpen="menuOpen" @toggle-menu="menuOpen = !menuOpen" />
...
export default {
  ...
  data() {
    return {
      ...
      isLoading: false,
      history: [],
      apiUrl: 'http://localhost/proyecto%20dise%C3%B1o%20web-semana5/proyecto%20dise%C3%B1o%20web-semana5/proyecto%20dise%C3%B1o%20web-seman3(1)/proyecto%20dise%C3%B1o%20web-semana2/proyecto%20dise%C3%B1o%20web/api'
    };
  },
  mounted() {
    setInterval(() => {
  ...
  methods: {
    ...
    async handleSendMessage(message) {
      ...
      try {
        const response = await fetch(`${this.apiUrl}/chat.php`, {
          method: 'POST',
        ...
        if (data.success) {
          ...
          this.messages.push({ type: 'bot', text: 'Lo siento, hubo un error. Intenta de nuevo.' });
        }
      } catch (error) {
    ...
    async loadHistory() {
      try {
        const response = await fetch(`${this.apiUrl}/get_chats.php`);
        const data = await response.json();
